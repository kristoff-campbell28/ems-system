...
public class Update {
	...
	public static void updater() {
		System.out.println("update things.");
		...
		if (MainClass.empHash.containsKey(employeeID)) {
			...
			System.out.println(
					"Please note that, due to safety reasons, " + "we only allow one item to be modified at a time.");
			// Print all the possible ones, with "1 for X idea" method.
			// Then do a switch statement for all of them. Break when done.
			...
			case 5:
				...
				System.out
						.println(ssnOld.substring(0, 3) + "-" + ssnOld.substring(3, 5) + "-" + ssnOld.substring(5, 8));
			...
			case 7:
				...
				// Bonus case for changing only first or only last name?
			...
			default:
				...
				// Make process go over again.
				break;
			}

		} else {
			System.out.println("Negative. Please enter a valid ID.");
			// Do some actual check here.
		}
